               ...
               GainControl
{
     ...
     * instance
     * @param mode Whether this controls audio INPUT or OUTPUT
     */
                              ...
                              Mode mode)
    {
        ...
        // Update the volume slider UI to reflect either the default level or a
        // level loaded from config
        fireVolumeChange();

        logger.debug("Created BVC " + this + " for Mode " + mode);
    }
    ...
    public void addGainChangeListener(GainChangeListener listener)
    {
        if(listener != null)
        {
            if(gainChangeListeners == null)
                gainChangeListeners = new ArrayList<>();
        ...
        try
        {
            ...
            if (cfg != null)
            {
                String volumeLevelString
                    = cfg.user().getString(volumeLevelConfigurationPropertyName);
    ...
    private float setVolumeLevel(float value)
    {
        ...
        if (cfg != null)
        {
            logger.debug("Setting volume property for " +
                                         volumeLevelConfigurationPropertyName);
            cfg.user().setProperty(
                    this.volumeLevelConfigurationPropertyName,
                    String.valueOf(volumeLevel));
        }
        else
        {
            logger.error("Unable to set volume property for " +
                                         volumeLevelConfigurationPropertyName);
        }
    ...
    /**
     * Returns the mode of the audio that this controls.
     *
     * @return the mode of the audio that this controls
     */
    public Mode getMode()
    {
        return mode;
    ...
    /**
     * The mode of the audio that this controls.
     */
    public enum Mode
    {
        INPUT,
