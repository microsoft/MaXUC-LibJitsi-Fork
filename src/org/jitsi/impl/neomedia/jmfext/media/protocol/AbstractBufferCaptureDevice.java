               ...
               Controls
{
     ...
     * </p>
     */
    private List<AbstractBufferStreamT> streams;
    ...
    {
        if (streams != null)
            for (AbstractBufferStreamT stream : streams)
                stream.start();
    }
        ...
        synchronized (streamsLock)
        {
            if (streams == null)
            {
                ...
                if (formatControls != null)
                {
                    ...
                    streams = new ArrayList<>(formatControlCount);

                    for (int i = 0; i < formatControlCount; i++)
                        streams.add(createStream(i, formatControls[i]));
            ...
            int streamCount = (streams == null) ? 0 : streams.size();

            SourceStreamT[] clone =
                (SourceStreamT[]) Array.newInstance(clz, streamCount);

            if (streamCount != 0)
                System.arraycopy(streams.toArray(), 0, clone, 0, streamCount);
            return clone;
        }
    }
    ...
    private Format internalGetFormat(int streamIndex, Format oldValue)
    {
        synchronized (streamsLock)
        {
            // If the stream is still being created, it won't exist in
            // streams yet - see getStreams().
            if (streams != null &&
                streams.size() > streamIndex)
            {
                AbstractBufferStreamT stream = streams.get(streamIndex);
     ...
     * data
     */
    List<AbstractBufferStreamT> streams()
    {
        synchronized (streamsLock)
        {
            return streams;
