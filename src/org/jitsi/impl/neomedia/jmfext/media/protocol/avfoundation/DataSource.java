...
import org.jitsi.impl.neomedia.avfoundation.*;
...
public class DataSource
    extends AbstractVideoPushBufferCaptureDevice<AVFoundationStream>
{
    /**
    ...
    /**
     * The <tt>AVCaptureSession</tt> which captures from {@link #device} and
    ...
    /**
     * The <tt>AVCaptureDevice</tt> which represents the media source of this
     * <tt>DataSource</tt>.
     */
     ...
     * the <tt>AVCaptureVideoDataOutput</tt> represented by the
     * <tt>AVFoundationStream</tt> made available by this <tt>DataSource</tt>.
     *
     * {@inheritDoc}
    ...
    protected FrameRateControl createFrameRateControl()
    {
        return
            new FrameRateControlAdapter()
            {
                 ...
                 * <tt>AVFoundationStream</tt> to delegate to.
                 */
                ...
                {
                    float frameRate = -1;
                    boolean frameRateFromAVFoundationStream = false;

                    synchronized (getStreamSyncRoot())
                    {
                        List<AVFoundationStream> streams = streams();

                        if (streams != null)
                        {
                            for (AVFoundationStream stream : streams)
                            {
                                if (stream != null)
                                {
                                    frameRate = stream.getFrameRate();
                                    frameRateFromAVFoundationStream = true;
                                    if (frameRate != -1)
                                        break;
                                }
                            }
                    ...
                    return
                        frameRateFromAVFoundationStream
                            ? frameRate
                            : this.frameRate;
                }
                ...
                public float setFrameRate(float frameRate)
                {
                    ...
                    synchronized (getStreamSyncRoot())
                    {
                        ...
                        if (streams != null)
                        {
                            for (AVFoundationStream stream : streams)
                            {
                                if (stream != null)
                                {
                                    float avFoundationStreamFrameRate
                                        = stream.setFrameRate(
                                                frameRate);

                                    if (avFoundationStreamFrameRate != -1)
                                    {
                                        setFrameRate
                                            = avFoundationStreamFrameRate;
                                    }
                                    frameRateFromAVFoundationStream = true;
                                }
                            }
                    ...
                    if (frameRateFromAVFoundationStream)
                        return setFrameRate;
                    else
                    {
     ...
     */
    @Override
    protected AVFoundationStream createStream(
            int streamIndex,
            FormatControl formatControl)
    {
        AVFoundationStream stream = new AVFoundationStream(this, formatControl);
        ...
        if (captureSession != null)
            ...
            {
                logger.error("Failed to addOutput to AVCaptureSession", nseex);
        ...
        throws IOException
    {
        ...
        if (!deviceIsOpened)
            throw new IOException("Failed to open AVCaptureDevice");
        ...
        /*
         * Add the AVCaptureOutputs represented by the AVFoundationStreams (if any)
            ...
            if (streams != null)
                for (AVFoundationStream stream : streams)
                    if (stream != null)
                        ...
                        {
                            captureSession
                                .addOutput(stream.captureOutput);
                        }
                        catch (NSErrorException nseex)
                        {
                            logger
                                .error(
                                    "Failed to addOutput to AVCaptureSession",
                                    nseex);
    ...
    /**
     * Sets the <tt>AVCaptureDevice</tt> which represents the media source of
     * this <tt>DataSource</tt>.
     *
     * @param device the <tt>AVCaptureDevice</tt> which represents the media
     * source of this <tt>DataSource</tt>
     */
    private void setDevice(AVCaptureDevice device)
    {
    ...
    public void setLocator(MediaLocator locator)
    {
        ...
        if ((locator != null)
                && VideoSystem.LOCATOR_PROTOCOL_AVFOUNDATION.equalsIgnoreCase(
                        locator.getProtocol()))
        {
            String deviceUID = locator.getRemainder();

            device = AVCaptureDevice.deviceWithUniqueID(deviceUID);
        }
        else
            device = null;
        setDevice(device);
