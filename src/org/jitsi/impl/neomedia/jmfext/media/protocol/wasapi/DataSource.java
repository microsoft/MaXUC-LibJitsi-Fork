...
public class DataSource
    extends AbstractPushBufferCaptureDevice<WASAPIStream>
{
    /**
    ...
    public DataSource(MediaLocator locator)
    {
        ...
        // We consider this DataSource as 'AEC enabled' if de-noise or echo
        // cancellation is on, unless there are no AEC supported formats
        // available (which probably indicates WASAPI failed to enable AEC
        // globally).
        aec = ((audioSystem.isDenoise() || audioSystem.isEchoCancel()) &&
               (audioSystem.getAECSupportedFormats().size() != 0));

        logger.debug("Data source " + (aec ? "does" : "doesn't") + " have AEC");
    }
