...
{
    /**
     * The <tt>Logger</tt> used by the <tt>WASAPI</tt> class and its
     * instances to log debug information.
     */
    private static final Logger logger = Logger.getLogger(WASAPI.class);
     ...
     */
    public static final List<Integer> HRESULT_BLOCKLIST;
    ...
    static
    {
        ...
        HRESULT_BLOCKLIST = Arrays.asList(AUDCLNT_E_DEVICE_INVALIDATED,
    ...
    /**
     * Stops iAudioClient. If iAudioClient is equal to 0 this method
     * does nothing.
     * @throws HResultException
     */
    public static void stopIAudioClient(long iAudioClient)
        throws HResultException
    {
        if(iAudioClient != 0)
        {
            IAudioClient_Stop(iAudioClient);
            logger.info("iAudioClient stopped with value " + iAudioClient);
        }
        else
        {
            logger.warn("iAudioClient = 0. Not calling IAudioClient_Stop");
        }
    }
