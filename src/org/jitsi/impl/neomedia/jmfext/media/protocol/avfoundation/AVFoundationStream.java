...
package org.jitsi.impl.neomedia.jmfext.media.protocol.avfoundation;
...
import org.jitsi.impl.neomedia.avfoundation.AVCaptureOutput;
import org.jitsi.impl.neomedia.avfoundation.AVCaptureVideoDataOutput;
import org.jitsi.impl.neomedia.avfoundation.CMSampleBuffer;
import org.jitsi.impl.neomedia.avfoundation.CVImageBuffer;
import org.jitsi.impl.neomedia.avfoundation.CVPixelBuffer;
import org.jitsi.impl.neomedia.avfoundation.CVPixelBufferAttributeKey;
import org.jitsi.impl.neomedia.avfoundation.CVPixelFormatType;
import org.jitsi.impl.neomedia.avfoundation.NSDictionary;
import org.jitsi.impl.neomedia.avfoundation.NSMutableDictionary;
...
/**
 * Implements a <tt>PushBufferStream</tt> using AVFoundation.
 *
 * @author Lyubomir Marinov
 */
public class AVFoundationStream
    extends AbstractPushBufferStream<DataSource>
{
    private static final Logger logger = Logger.getLogger(AVFoundationStream.class);
     ...
     * ourselves because AVFoundation will buffer them all and the video will
     * be late.
     */
    private final boolean alwaysDiscardsLateVideoFrames;
    ...
    /**
     * The <tt>AVCaptureOutput</tt> represented by this <tt>SourceStream</tt>.
     */
    final AVCaptureVideoDataOutput captureOutput
     ...
     * <tt>AVFoundationStream</tt> to provide the latest available frame and not
     * wait for AVFoundation to capture a new one.
     */
     ...
     * <tt>AVFoundationStream</tt> so that the call is not made in AVFoundation
     * and we can drop late frames when
     * {@link #alwaysDiscardsLateVideoFrames} is <tt>false</tt>.
     */
    private Thread transferDataThread;

    /**
     * Initializes a new <tt>AVFoundationStream</tt> instance which is to have its
     ...
     */
    AVFoundationStream(DataSource dataSource, FormatControl formatControl)
    {
        ...
        captureOutput.setDelegate(
                new AVCaptureVideoDataOutput.Delegate()
                {
                    /**
                     * Notifies this <tt>Delegate</tt> that the
                     * <tt>AVCaptureOutput</tt> to which it is set has output a
                     * specific <tt>CVImageBuffer</tt> representing a video
                     * frame with a specific <tt>CMSampleBuffer</tt>.
                     *
                     * @param videoFrame the <tt>CVImageBuffer</tt> which
                     * represents the output video frame
                     * @param sampleBuffer the <tt>CMSampleBuffer</tt> which
    ...
    /**
     * Notifies this instance that its <tt>AVCaptureOutput</tt> has output a
     * specific <tt>CVImageBuffer</tt> representing a video frame with a
     * specific <tt>CMSampleBuffer</tt>.
     *
     * @param captureOutput the <tt>AVCaptureOutput</tt> which has output a
     * video frame
            ...
            CMSampleBuffer sampleBuffer)
    {
        ...
        {
            if (!alwaysDiscardsLateVideoFrames && (data != null))
            {
                if (nextData != null)
                {
                    nextData.free();
                ...
                nextData = byteBufferPool.getBuffer(pixelBuffer.getByteBufferSize());
                if (nextData != null)
                {
                    nextData.setLength(
                            pixelBuffer.getBytes(
                                    nextData.getPtr().address(),
            ...
            data = byteBufferPool.getBuffer(pixelBuffer.getByteBufferSize());
            if (data != null)
            {
                data.setLength(
                        pixelBuffer.getBytes(
            ...
            if (alwaysDiscardsLateVideoFrames)
    ...
    private Format getCaptureOutputFormat()
    {
        ...
        if (pixelBufferAttributes != null)
        {
            ...
            logger.debug("pixelBufferAttributes < formatType: " +
                                 pixelFormatType + ", width: " + width +
                                 ", height: " + height + ">");
        ...
        {
            logger.warn("pixelBufferAttributes are null");
        }
     ...
     * <tt>AVCaptureVideoDataOutput</tt> represented by this
     * <tt>AVFoundationStream</tt>.
     *
    ...
    {
        return (float) (1.0d / captureOutput.videoMinFrameDuration());
    }
    ...
    public void read(Buffer buffer)
    {
        synchronized (dataSyncRoot)
        {
            ...
            {
                if (AVFrameWrapper.read(buffer, format, data) < 0)
                    data.free();
                /*
            ...
            else
            {
                ...
                CVPixelBuffer.memcpy(bytes, 0, length, data.getPtr().address());
                data.free();
                data = null;
     ...
     * AVFoundation and we can drop late frames in the meantime.
     */
    ...
    {
        VideoFormat videoFormat = (VideoFormat) format;
        logger.debug("Setting output format to " + videoFormat);
         ...
         * AVCaptureDevice in its formatDescriptions early in its creation.
        ...
        if (size == null)
        {
            ...
            logger.debug("No size provided - using default height: " +
        ...
        else
        {
            width = size.width;
            height = size.height;
            logger.debug("Video size specified - height: " + height +
                                 " and width: " + width);
        }
        ...
        if (format instanceof AVFrameFormat)
        {
            ...
            logger.debug("Matched pixFmt " + pixFmt + " to encoding " + encoding);
        }
    ...
    {
        captureOutput.setVideoMinFrameDuration(1.0d / frameRate);
        return getFrameRate();
    }
