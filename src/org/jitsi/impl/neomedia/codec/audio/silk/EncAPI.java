...
public class EncAPI
{
    ...
    (
        SKP_Silk_encoder_state_FLP        encState,          /* I/O: State                                           */
        SKP_SILK_SDK_EncControlStruct     encStatus,         /* O:   Control structure                               */
        final EncodeMem                   mem
    )
    {
        ...
        /* Reset Encoder */
        if( (ret += InitEncoderFLP.SKP_Silk_init_encoder_FLP( encState, mem )) != 0 )
        {
            assert( false );
        }
    ...
    )
    {
        ...
        if( ( ret = ControlCodecFLP.SKP_Silk_control_encoder_FLP( psEnc, API_fs_Hz, max_internal_fs_kHz, PacketSize_ms, TargetRate_bps,
                        PacketLoss_perc, UseInBandFEC, UseDTX, input_ms, Complexity, mem ) ) != 0 )
        {
            assert( false );
            return( ret );
        }
        ...
        {
            DetectSWBInput.SKP_Silk_detect_SWB_input( psEnc.sCmn.sSWBdetect, samplesIn,samplesIn_offset, nSamplesIn, mem );
        }
        ...
        while( true )
        {
            ...
            else
            {
                ...
                ret += Resampler.SKP_Silk_resampler(psEnc.sCmn.resampler_state,
                        psEnc.sCmn.inputBuf, psEnc.sCmn.inputBufIx, samplesIn, samplesIn_offset, nSamplesFromInput, mem );
            }
            ...
            if( psEnc.sCmn.inputBufIx >= psEnc.sCmn.frame_length )
            {
                ...
                if( MaxBytesOut == 0 )
                {
                    ...
                    mem.SKP_Silk_SDK_Encode__tmp_short[0] = MaxBytesOut;
                    if( ( ret = EncodeFrameFLP.SKP_Silk_encode_frame_FLP( psEnc, outData, outData_offset,
                            mem.SKP_Silk_SDK_Encode__tmp_short, psEnc.sCmn.inputBuf, mem ) ) != 0 )
                    {
                        assert( false );
                    }
                    MaxBytesOut = mem.SKP_Silk_SDK_Encode__tmp_short[0];
                }
                else
                {
                    /* outData already contains a payload */
                    if( ( ret = EncodeFrameFLP.SKP_Silk_encode_frame_FLP( psEnc, outData, outData_offset,
                           nBytesOut, psEnc.sCmn.inputBuf, mem) ) != 0 )
                    {
                        assert( false );
                    }
