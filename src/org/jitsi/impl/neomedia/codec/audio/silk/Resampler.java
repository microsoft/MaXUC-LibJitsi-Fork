...
public class Resampler
{
    ...
    )
    {
        ...
        if( S.nPreDownsamplers + S.nPostUpsamplers > 0 ) {
            ...
            short[]   in_buf = mem.zero(mem.SKP_Silk_resampler__in_buf);
            short[]   out_buf = mem.zero(mem.SKP_Silk_resampler__out_buf);
            ...
            while( inLen > 0 ) {
                ...
                if( S.nPreDownsamplers > 0 ) {
                    ...
                    if( S.nPostUpsamplers > 0 ) {
                        S.resampler_function(S, out_buf, 0, in_buf, 0, ( nSamplesIn >> S.nPreDownsamplers ), mem );
                        S.up_post_function(S.sUpPost, out, out_offset, out_buf, 0, ( nSamplesOut >> S.nPostUpsamplers ));
                    } else {
                        S.resampler_function( S, out, out_offset, in_buf, 0, ( nSamplesIn >> S.nPreDownsamplers ), mem );
                    }
                } else {
                    S.resampler_function( S, out_buf, 0, in, in_offset, ( nSamplesIn >> S.nPreDownsamplers ), mem );
                    S.up_post_function( S.sUpPost, out, out_offset, out_buf, 0, ( nSamplesOut >> S.nPostUpsamplers ));
                }
            ...
            S.resampler_function( S, out, out_offset, in, in_offset, inLen, mem );
        }
        return 0;
    }
}
...
class ResamplerImplWrapper implements ResamplerFP
{
    public void resampler_function(Object state, short[] out, int outOffset,
            short[] in, int inOffset, int len, BaseMem mem) {
        ResamplerPrivateUp2HQ.SKP_Silk_resampler_private_up2_HQ_wrapper(state,
                out, outOffset, in, inOffset, len, mem);
    }
}
...
class ResamplerImplCopy implements ResamplerFP
{
    public void resampler_function(Object state, short[] out, int outOffset,
            short[] in, int inOffset, int len, BaseMem mem) {
        ResamplerPrivateCopy.SKP_Silk_resampler_private_copy(state, out, outOffset, in, inOffset, len, mem);
    }
}
/*----------------------------------------------------------------*/
