...
public class ShellCoder
{
    ...
    )
    {
        int k;
        for( k = 0; k < len; k++ ) {
            out[ k ] = in[ in_offset + 2 * k ] + in[ in_offset + 2 * k + 1 ];
        }
    }
    ...
    {
        int[] pulses1 = mem.zero(mem.SKP_Silk_shell_encoder__pulses1);
        int[] pulses2 = mem.zero(mem.SKP_Silk_shell_encoder__pulses2);
        int[] pulses3 = mem.zero(mem.SKP_Silk_shell_encoder__pulses3);
        int[] pulses4 = mem.zero(mem.SKP_Silk_shell_encoder__pulses4);

        /* this function operates on one shell code frame of 16 pulses */
        assert( Define.SHELL_CODEC_FRAME_LENGTH == 16 );

        /* tree representation per pulse-subframe */
        combine_pulses( pulses1, pulses0,pulses0_offset, 8 );
    ...
    {
        int[] pulses3 = mem.zero(mem.SKP_Silk_shell_decoder__pulses3);
        int[] pulses2 = mem.zero(mem.SKP_Silk_shell_decoder__pulses2);
        int[] pulses1 = mem.zero(mem.SKP_Silk_shell_decoder__pulses1);
