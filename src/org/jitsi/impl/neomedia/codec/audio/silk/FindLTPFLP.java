...
public class FindLTPFLP
{
    ...
    )
    {
        ...
        float[] d = mem.zero(mem.SKP_Silk_find_LTP_FLP__d);
        float[] delta_b = mem.zero(mem.SKP_Silk_find_LTP_FLP__delta_b);
        float[] w = mem.zero(mem.SKP_Silk_find_LTP_FLP__w);
        float[] nrg = mem.zero(mem.SKP_Silk_find_LTP_FLP__nrg);
        float[] Rr = mem.zero(mem.SKP_Silk_find_LTP_FLP__Rr);
        float[] rr = mem.zero(mem.SKP_Silk_find_LTP_FLP__rr);
        float m, g;
        float regu;
        float r_ptr[], lag_ptr[];
        ...
        for( k = 0; k < Define.NB_SUBFR; k++ ) {
            ...
            SolveLSFLP.SKP_Silk_solve_LDL_FLP( WLTP_ptr, WLTP_ptr_offset, Define.LTP_ORDER, Rr, b_ptr, b_ptr_offset, mem );
