...
{
    final int RandState[] = new int[ Define.DECISION_DELAY ];
    final int Q_Q10[]     = new int[ Define.DECISION_DELAY ];
    final int Xq_Q10[]    = new int[ Define.DECISION_DELAY ];
    final int Pred_Q16[]  = new int[ Define.DECISION_DELAY ];
    final int Shape_Q10[] = new int[ Define.DECISION_DELAY ];
    final int Gain_Q16[]  = new int[ Define.DECISION_DELAY ];
    final int sLPC_Q14[]  = new int[ Define.MAX_FRAME_LENGTH / Define.NB_SUBFR + Define.NSQ_LPC_BUF_LENGTH() ];
    int LF_AR_Q12;
    int Seed;
    int SeedInit;
    int RD_Q10;
...
class NSQ_sample_struct
{
    ...
    /**
     * Copy all fields from one instance of this class to another.
     * @param input The input class.  Copied to this instance.
     */
    public void copyFrom(NSQ_sample_struct input)
    {
        Q_Q10 = input.Q_Q10;
        RD_Q10 = input.RD_Q10;
        xq_Q14 = input.xq_Q14;
        LF_AR_Q12 = input.LF_AR_Q12;
        sLTP_shp_Q10 = input.sLTP_shp_Q10;
        LPC_exc_Q16 = input.LPC_exc_Q16;
    }
}
...
public class NSQDelDec
{
    static void SKP_Silk_NSQ_del_dec(
        ...
        final short               x[],                /* I    Prefiltered input signal            */
    ...
    )
    {
        ...
        short[] pxq;
        int     pxq_offset;
        int   sLTP_Q16[] = mem.zero(mem.SKP_Silk_NSQ_del_dec__sLTP_Q16);
        short sLTP[] = mem.zero(mem.SKP_Silk_NSQ_del_dec__sLTP);
        int   HarmShapeFIRPacked_Q14;
        int   offset_Q10;
        int   FiltState[] = mem.zero(mem.SKP_Silk_NSQ_del_dec__FiltState);
        int   RDmin_Q10;
        int   x_sc_Q10[] = mem.zero(mem.SKP_Silk_NSQ_del_dec__x_sc_Q10);

        NSQDelDecStruct psDelDec[] = mem.SKP_Silk_NSQ_del_dec__psDelDec;
        for(int psDelDecIni_i=0; psDelDecIni_i<Define.DEL_DEC_STATES_MAX; psDelDecIni_i++)
        {
            psDelDec[psDelDecIni_i].zero();
        }
        ...
        for( k = 0; k < Define.NB_SUBFR; k++ ) {
            ...
            SKP_Silk_nsq_del_dec_scale_states( NSQ, psDelDec, x, x_offset, x_sc_Q10,
                subfr_length, sLTP, sLTP_Q16, k, psEncC.nStatesDelayedDecision,
                LTP_scale_Q14, Gains_Q16, psEncCtrlC.pitchL );

            mem.SKP_Silk_NSQ_del_dec__smpl_buf_idx_ptr[0] = smpl_buf_idx;
            SKP_Silk_noise_shape_quantizer_del_dec( NSQ, psDelDec, psEncCtrlC.sigtype, x_sc_Q10, q, q_offset, pxq, pxq_offset,
                ...
                psEncC.nStatesDelayedDecision, mem.SKP_Silk_NSQ_del_dec__smpl_buf_idx_ptr, decisionDelay, mem );
            smpl_buf_idx = mem.SKP_Silk_NSQ_del_dec__smpl_buf_idx_ptr[0];

            x_offset   += psEncC.subfr_length;
            q_offset   += psEncC.subfr_length;
    ...
    static void SKP_Silk_noise_shape_quantizer_del_dec(
        SKP_Silk_nsq_state  NSQ,                    /* I/O  NSQ state                           */
        NSQDelDecStruct     psDelDec[],             /* I/O  Delayed decision states             */
    ...
    )
    {
        ...
        NSQ_sample_struct[][]  psSampleState = mem.SKP_Silk_noise_shape_quantizer_del_dec__psSampleState;
        for(int Ini_i=0; Ini_i<Define.DEL_DEC_STATES_MAX; Ini_i++)
        {
            for(int Ini_j=0; Ini_j<2; Ini_j++)
            {
                psSampleState[Ini_i][Ini_j].zero();
            }
        }
        NSQDelDecStruct psDD;
...
//                psSS                                     = &psSampleState[ k ][ 0 ];
    ...
    static void SKP_Silk_copy_del_dec_state(
            NSQDelDecStruct  DD_dst,                /* I    Dst del dec state                   */
