...
public class PLC
{
    ...
    static final short[] HARM_ATT_Q15              = { 32440, 31130 }; /* 0.99, 0.95 */
    static final short[] PLC_RAND_ATTENUATE_V_Q15  = { 31130, 26214 }; /* 0.95, 0.8 */
    static final short[] PLC_RAND_ATTENUATE_UV_Q15 = { 32440, 29491 }; /* 0.99, 0.9 */
    ...
    )
    {
        ...
        if( lost != 0 ) {
            ...
            /* Generate Signal          */
            /****************************/
            SKP_Silk_PLC_conceal( psDec, psDecCtrl, signal, signal_offset, mem );
        } else {
            /****************************/
            /* Update state             */
            /****************************/
    ...
    )
    {
        int   i, j, k;
        short[] B_Q14;
        short[] exc_buf = mem.zero(mem.SKP_Silk_PLC_conceal__exc_buf);
        short[] exc_buf_ptr;
        int     exc_buf_ptr_offset;

        short rand_scale_Q14;
        short[] A_Q12_tmp = mem.zero(mem.SKP_Silk_PLC_conceal__A_Q12_tmp);

        int rand_seed, harm_Gain_Q15, rand_Gain_Q15;
        int   lag, idx, shift1, shift2;
        int[] shift1_ptr = mem.zero(mem.SKP_Silk_PLC_conceal__shift1_ptr);
        int[] shift2_ptr = mem.zero(mem.SKP_Silk_PLC_conceal__shift2_ptr);

        int energy1, energy2;
        int[] energy1_ptr = mem.zero(mem.SKP_Silk_PLC_conceal__energy1_ptr);
        int[] energy2_ptr = mem.zero(mem.SKP_Silk_PLC_conceal__energy2_ptr);

        int[]  rand_ptr, pred_lag_ptr;
        int    rand_ptr_offset, pred_lag_ptr_offset;

        int[] sig_Q10 = mem.zero(mem.SKP_Silk_PLC_conceal__sig_Q10);
        int[] sig_Q10_ptr;
        ...
        if( psDec.lossCnt == 0 ) {
            ...
            if( psDec.prev_sigtype == Define.SIG_TYPE_UNVOICED ) {
                ...
                mem.SKP_Silk_PLC_conceal__tmp_int[0] = 0;
                LPCInvPredGain.SKP_Silk_LPC_inverse_pred_gain( mem.SKP_Silk_PLC_conceal__tmp_int, psPLC.prevLPC_Q12, psDec.LPC_order, mem );
                invGain_Q30 = mem.SKP_Silk_PLC_conceal__tmp_int[0];
...
//TODO: array bonds check???
    ...
    )
    {
        int   i, energy_shift;
        int   energy;
        int[] energy_ptr = mem.zero(mem.SKP_Silk_PLC_glue_frames__energy_ptr);
        int[] energy_shift_ptr = mem.zero(mem.SKP_Silk_PLC_glue_frames__energy_shift_ptr);
        ...
        if( psDec.lossCnt != 0) {
            ...
            int[] conc_energy_ptr = mem.zero(mem.SKP_Silk_PLC_glue_frames__conc_energy_ptr);
            int[] conc_energy_shift_ptr = mem.zero(mem.SKP_Silk_PLC_glue_frames__conc_energy_shift_ptr);
            ...
            if( psDec.sPLC.last_frame_lost != 0 ) {
                ...
                if( energy > psPLC.conc_energy ) {
                    ...
                    gain_Q12 = Inlines.SKP_Silk_SQRT_APPROX( frac_Q24, mem );
