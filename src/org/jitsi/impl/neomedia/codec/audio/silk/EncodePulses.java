...
public class EncodePulses
{
    ...
    )
    {
        ...
        int[]   abs_pulses = mem.zero(mem.SKP_Silk_encode_pulses__abs_pulses);
        int[]   sum_pulses = mem.zero(mem.SKP_Silk_encode_pulses__sum_pulses);
        int[]   nRshifts   = mem.zero(mem.SKP_Silk_encode_pulses__nRshifts);
        int[]   pulses_comb = mem.zero(mem.SKP_Silk_encode_pulses__pulses_comb);
        int   []abs_pulses_ptr;
        ...
        for( i = 0; i < iter; i++ ) {
            if( sum_pulses[ i ] > 0 ) {
                ShellCoder.SKP_Silk_shell_encoder( psRC, abs_pulses, i * Define.SHELL_CODEC_FRAME_LENGTH, mem );
            }
        }
        ...
        /* Encode signs */
        /****************/
        CodeSigns.SKP_Silk_encode_signs( psRC, q, frame_length, sigtype, QuantOffsetType, RateLevelIndex, mem );
    }
}
