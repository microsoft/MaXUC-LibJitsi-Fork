...
public class DecodePulses
{
    ...
    )
    {
        ...
        int[]   sum_pulses = mem.zero(mem.SKP_Silk_decode_pulses__sum_pulses);
        int[]   nLshifts = mem.zero(mem.SKP_Silk_decode_pulses__nLshifts);
        int[]   pulses_ptr;
        ...
        /* Decode rate level */
        /*********************/
        mem.SKP_Silk_decode_pulses__tmp_int[0] = psDecCtrl.RateLevelIndex;
        RangeCoder.SKP_Silk_range_decoder( mem.SKP_Silk_decode_pulses__tmp_int, 0, psRC,
                TablesPulsesPerBlock.SKP_Silk_rate_levels_CDF[ psDecCtrl.sigtype ], 0, TablesPulsesPerBlock.SKP_Silk_rate_levels_CDF_offset );
        psDecCtrl.RateLevelIndex = mem.SKP_Silk_decode_pulses__tmp_int[0];
        ...
        for( i = 0; i < iter; i++ ) {
            if( sum_pulses[ i ] > 0 ) {
                ShellCoder.SKP_Silk_shell_decoder( q, Macros.SKP_SMULBB( i, Define.SHELL_CODEC_FRAME_LENGTH ), psRC, sum_pulses[ i ], mem );
            } else {
        ...
        for( i = 0; i < iter; i++ ) {
            if( nLshifts[ i ] > 0 ) {
                ...
                for( k = 0; k < Define.SHELL_CODEC_FRAME_LENGTH; k++ ) {
                    ...
                    for( j = 0; j < nLS; j++ ) {
                        abs_q = abs_q << 1;
                        mem.SKP_Silk_decode_pulses__tmp_int[0] = 0;
                        RangeCoder.SKP_Silk_range_decoder( mem.SKP_Silk_decode_pulses__tmp_int, 0, psRC, TablesOther.SKP_Silk_lsb_CDF, 0, 1 );
                        bit = mem.SKP_Silk_decode_pulses__tmp_int[0];
                        abs_q += bit;
                    }
        ...
        CodeSigns.SKP_Silk_decode_signs( psRC, q, frame_length, psDecCtrl.sigtype,
                psDecCtrl.QuantOffsetType, psDecCtrl.RateLevelIndex, mem);
    }
}
