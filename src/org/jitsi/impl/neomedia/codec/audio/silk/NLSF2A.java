...
public class NLSF2A
{
        ...
        )
    {
        ...
        for( k = 1; k < dd; k++ ) {
            ...
            for( n = k; n > 1; n-- ) {
                out[n] += out[n-2] - (int)SigProcFIX.SKP_RSHIFT_ROUND64( SigProcFIX.SKP_SMULL( ftmp , out[n-1] ), 20 );
            }
            out[1] -= ftmp;
        }
    }
    ...
    {
        int k, i, dd;
        int[] cos_LSF_Q20 = mem.zero(mem.SKP_Silk_NLSF2A__cos_LSF_Q20);

        int[] P = mem.zero(mem.SKP_Silk_NLSF2A__P);
        int[] Q = mem.zero(mem.SKP_Silk_NLSF2A__Q);

        int Ptmp, Qtmp;
        int f_int;
        int f_frac;
        int cos_val, delta;
        int[] a_int32 = mem.zero(mem.SKP_Silk_NLSF2A__a_int32);
        ...
        for( k = 0; k < d; k++ ) {
            ...
            delta   = LSFCosTable.SKP_Silk_LSFCosTab_FIX_Q12[ f_int + 1 ] - cos_val;  /* Q12, with a range of 0..200 */
