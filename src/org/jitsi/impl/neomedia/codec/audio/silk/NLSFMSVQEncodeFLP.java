...
public class NLSFMSVQEncodeFLP
{
    ...
    )
    {
        ...
        float[] pNLSF_in = mem.zero(mem.SKP_Silk_NLSF_MSVQ_encode_FLP__pNLSF_in);

        float[]   pRateDist = mem.zero(mem.SKP_Silk_NLSF_MSVQ_encode_FLP__pRateDist);
        float[]   pRate = mem.zero(mem.SKP_Silk_NLSF_MSVQ_encode_FLP__pRate);
        float[]   pRate_new = mem.zero(mem.SKP_Silk_NLSF_MSVQ_encode_FLP__pRate_new);
        int[]     pTempIndices = mem.zero(mem.SKP_Silk_NLSF_MSVQ_encode_FLP__pTempIndices);
        int[]     pPath = mem.zero(mem.SKP_Silk_NLSF_MSVQ_encode_FLP__pPath);
        int[]     pPath_new = mem.zero(mem.SKP_Silk_NLSF_MSVQ_encode_FLP__pPath_new);
        float[]   pRes = mem.zero(mem.SKP_Silk_NLSF_MSVQ_encode_FLP__pRes);
        float[]   pRes_new = mem.zero(mem.SKP_Silk_NLSF_MSVQ_encode_FLP__pRes_new);
        ...
        for( s = 0; s < psNLSF_CB_FLP.nStages; s++ ) {
            ...
            NLSFVQRateDistortionFLP.SKP_Silk_NLSF_VQ_rate_distortion_FLP( pRateDist, pCurrentCBStage,
                    pRes, pW, pRate, NLSF_mu, prev_survivors, LPC_order, mem );

            /* Sort the rate-distortion errors */
            SortFLP.SKP_Silk_insertion_sort_increasing_FLP( pRateDist, pTempIndices, prev_survivors * pCurrentCBStage.nVectors, cur_survivors );
        ...
        if( deactivate_fluc_red != 1 ) {
            ...
            for( s = 0; s < cur_survivors; s++ ) {
                ...
                NLSFMSVQDecodeFLP.SKP_Silk_NLSF_MSVQ_decode_FLP( pNLSF, psNLSF_CB_FLP,
                        pPath_new, s * psNLSF_CB_FLP.nStages, LPC_order, mem );
        ...
        NLSFMSVQDecodeFLP.SKP_Silk_NLSF_MSVQ_decode_FLP( pNLSF, psNLSF_CB_FLP, NLSFIndices, 0, LPC_order, mem );
    }
}
