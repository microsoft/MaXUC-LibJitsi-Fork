...
public class PitchAnalysisCoreFLP
{
    ...
    {
        float[] signal_8kHz = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__signal_8kHz);
        float[] signal_4kHz = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__signal_4kHz);
        float[] scratch_mem = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__scratch_mem);
        float[] filt_state = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__filt_state);
        int   i, k, d, j;
        float threshold, contour_bias;
        float[][] C = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__C);
        float[] CC = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__CC);
        float[] target_ptr, basis_ptr;
        int target_ptr_offset, basis_ptr_offset;
        double    cross_corr, normalizer, energy, energy_tmp;
        int[]   d_srch = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__d_srch);
        short[] d_comp = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__d_comp);
        ...
        // These two are initalized in loops below.
        float[][][] energies_st3 = mem.SKP_Silk_pitch_analysis_core_FLP__energies_st3;
        float[][][] cross_corr_st3 = mem.SKP_Silk_pitch_analysis_core_FLP__cross_corr_st3;

        int diff, lag_counter;
        int frame_length, frame_length_8kHz, frame_length_4kHz;
        int sf_length, sf_length_8kHz;
        ...
        assert( complexity >= SigProcFIX.SKP_Silk_PITCH_EST_MIN_COMPLEX );
        assert( complexity <= SigProcFIX.SKP_Silk_PITCH_EST_MAX_COMPLEX );
        ...
        if( Fs_kHz == 12 )
        {
            short[] signal_12 = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__signal_12);
            short[] signal_8 = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__signal_8);
            int[] R23 = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__R23);

            /* Resample to 12 -> 8 khz */
            for(int i_djinn=0; i_djinn<6; i_djinn++)
                R23[i_djinn] = 0;
            SigProcFLP.SKP_float2short_array( signal_12, signal, CommonPitchEstDefines.PITCH_EST_FRAME_LENGTH_MS * 12);
            ResamplerDown23.SKP_Silk_resampler_down2_3( R23,0, signal_8,0, signal_12,0, CommonPitchEstDefines.PITCH_EST_FRAME_LENGTH_MS * 12, mem );
        ...
        else if( Fs_kHz == 16 )
        {
            if( complexity == SigProcFIX.SKP_Silk_PITCH_EST_MAX_COMPLEX )
            {
                ...
                Decimate2CoarseFLP.SKP_Silk_decimate2_coarse_FLP( signal, filt_state, signal_8kHz,
            ...
            else
            {
                ...
                Decimate2CoarsestFLP.SKP_Silk_decimate2_coarsest_FLP( signal, filt_state, signal_8kHz,
                    scratch_mem, frame_length_8kHz );
            }
        }
        else if( Fs_kHz == 24 )
        {
            short[] signal_24 = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__signal_24);
            short[] signal_8 = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__signal_8);
            int[] filt_state_fix = mem.zero(mem.SKP_Silk_pitch_analysis_core_FLP__filt_state_fix);

            /* Resample to 24 -> 8 khz */
            SigProcFLP.SKP_float2short_array( signal_24, signal, 24 * CommonPitchEstDefines.PITCH_EST_FRAME_LENGTH_MS );
            for(int i_djinn=0; i_djinn<8; i_djinn++)
                filt_state_fix[i_djinn] = 0;
            ResamplerDown3.SKP_Silk_resampler_down3( filt_state_fix,0, signal_8,0, signal_24,0, 24 * CommonPitchEstDefines.PITCH_EST_FRAME_LENGTH_MS, mem );
            SigProcFLP.SKP_short2float_array( signal_8kHz,0, signal_8,0, frame_length_8kHz );
        }
        else
        {
        ...
        if( complexity == SigProcFIX.SKP_Silk_PITCH_EST_MAX_COMPLEX )
        {
            ...
            for(int i_djinn=0; i_djinn<4; i_djinn++)
                filt_state[i_djinn] = 0;
            Decimate2CoarseFLP.SKP_Silk_decimate2_coarse_FLP( signal_8kHz, filt_state,
        ...
        else
        {
            ...
            for(int i_djinn=0; i_djinn<4; i_djinn++)
                filt_state[i_djinn] = 0;
            Decimate2CoarsestFLP.SKP_Silk_decimate2_coarsest_FLP( signal_8kHz, filt_state,
                signal_4kHz, scratch_mem, frame_length_4kHz );
        }
...
//                        basis_ptr[ sf_length_8kHz ] * basis_ptr[ sf_length_8kHz ];
        ...
        *********************************************************************************/
        ...
        for( k = 0; k < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; k++ )
        {
            ...
            energy_tmp = EnergyFLP.SKP_Silk_energy_FLP( target_ptr,target_ptr_offset, sf_length_8kHz );
        ...
        if( Fs_kHz > 8 ) {
            ...
            SKP_P_Ana_calc_corr_st3( cross_corr_st3, signal, start_lag, sf_length, complexity, mem );
            SKP_P_Ana_calc_energy_st3( energies_st3, signal, start_lag, sf_length, complexity, mem );
            ...
            cbk_offset = PitchEstTables.SKP_Silk_cbk_offsets_stage3[ complexity ];
         ...
         **********************************************************************/
    {
        ...
        float[] scratch_mem = mem.zero(mem.SKP_P_Ana_calc__scratch_mem);
        ...
        target_ptr_offset = ( sf_length << 2 );
        for( k = 0; k < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; k++ )
        {
            lag_counter = 0;

            /* Calculate the correlations for each subframe */
            for( j = PitchEstTables.SKP_Silk_Lag_range_stage3[ complexity ][ k ][ 0 ]; j <= PitchEstTables.SKP_Silk_Lag_range_stage3[ complexity ][ k ][ 1 ]; j++ )
            {
                basis_ptr = target_ptr;
            ...
            delta = PitchEstTables.SKP_Silk_Lag_range_stage3[ complexity ][ k ][ 0 ];
    ...
    ****************************************************************/
    {
        ...
        for( k = 0; k < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; k++ )
        {
            ...
            basis_ptr = target_ptr;
            basis_ptr_offset = target_ptr_offset - ( start_lag + PitchEstTables.SKP_Silk_Lag_range_stage3[complexity ][ k ][ 0 ]);
            energy = EnergyFLP.SKP_Silk_energy_FLP( basis_ptr,basis_ptr_offset, sf_length ) + 1e-3;
            assert( energy >= 0.0 );
            scratch_mem[lag_counter] = (float)energy;
            lag_counter++;

            for( i = 1; i < ( PitchEstTables.SKP_Silk_Lag_range_stage3[ complexity ][ k ][ 1 ] - PitchEstTables.SKP_Silk_Lag_range_stage3[ complexity ][ k ][ 0 ] + 1 ); i++ )
            {
