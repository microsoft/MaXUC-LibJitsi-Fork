    ...
    implements PushBufferStream
{
    ...
    private static class InSampleDesc
    {
        ...
        {
            this.buffer
                = (buffer == null) ? null : new SoftReference<>(buffer);
        }
        ...
        throws IOException
    {
        ...
        {
            AudioMixingPushBufferStream[] existingStreams = outStreams.toArray(
                           new AudioMixingPushBufferStream[outStreams.size()]);

            logger.debug(this + "addOutStream called with" + outStream);
            logger.debug(this + "addOutStream existing streams=" +
                         Arrays.toString(existingStreams));
        ...
        throws IOException
    {
        ...
        synchronized (outStreams)
        {
            ...
            logger.debug(this + "removeOutStream called with" + outStream);
            logger.debug(this + "removeOutStream existing streams=" +
                         Arrays.toString(existingStreams));
        ...
        if (valueIsChanged)
        {
            ...
            for (InStreamDesc inStreamDesc : newValue)
            {
                ...
                if (!(inStream instanceof CachingPushBufferStream))
                {
                    ...
                    logger.trace("Created CachingPushBufferStream"
                                    + " with hashCode "
    ...
    protected void transferData(Buffer buffer)
    {
        ...
        Log.logReceivedBytes(this, maxInSampleCount);
