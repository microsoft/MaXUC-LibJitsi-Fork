...
public class SRTPCryptoContext
{
    ...
    public void transformPacket(RawPacket pkt)
    {
        ...
        {
            authenticatePacketHMCSHA1(pkt, rolloverCounter);
            pkt.append(tagStore, policy.getAuthTagLength());
        }

        /* Update the rollover counter if necessary */
    ...
    public boolean reverseTransformPacket(RawPacket pkt)
    {
        ...
        // Stores the guessed rollover counter in this.guessedROC
    ...
    public void processPacketAESCM(RawPacket pkt)
    {
        ...
        int seqNo = pkt.getSequenceNumber();
        long index = ((long) this.rolloverCounter << 16) | seqNo;
    ...
    public void processPacketAESF8(RawPacket pkt)
    {
        ...
        // set the rollover counter in network order into IV
        ivStore[12] = (byte) (this.rolloverCounter >> 24);
        ivStore[13] = (byte) (this.rolloverCounter >> 16);
        ivStore[14] = (byte) (this.rolloverCounter >> 8);
        ivStore[15] = (byte) this.rolloverCounter;
     ...
     *
     * This method supports a 64 packet history relative the given
     * sequence number.
     *
     ...
     * @param guessedIndex guessed rollover counter
    ...
    boolean checkReplay(int seqNo, long guessedIndex)
    {
        ...
        long localIndex = (((long) this.rolloverCounter) << 16) | this.seqNum;
    ...
    public void deriveSrtpKeys(long index)
    {
        ...
        if (authKey != null)
        {
            ...
            case SRTPPolicy.HMACSHA1_AUTHENTICATION:
                ...
                break;
            default:
                   logger.warn("Unhandled authentication type was provided: " + policy.getAuthType());
            }
    ...
    private long guessIndex(int seqNo)
    {
        if (this.seqNum < 32768)
        {
            ...
            {
                guessedROC = rolloverCounter - 1;
            }
            else
            {
                guessedROC = rolloverCounter;
            }
        }
        else
        {
     ...
     * @param guessedIndex guessed index
     */
    private void update(int seqNo, long guessedIndex)
    {
        long delta = guessedIndex - (((long) this.rolloverCounter) << 16 | this.seqNum);
        ...
        if (this.guessedROC > this.rolloverCounter)
        {
     ...
     * @param rolloverCounter
     *            The Roll-Over-Counter for this context
     * @param deriveRate
     *            The key derivation rate for this context
     * @return a new SRTPCryptoContext with all relevant data set.
     */
    public SRTPCryptoContext deriveContext(long ssrc, int rolloverCounter, long deriveRate)
    {
        SRTPCryptoContext pcc = null;
        pcc = new SRTPCryptoContext(ssrc, rolloverCounter, deriveRate, masterKey,
                masterSalt, policy);
        return pcc;
    }
}
