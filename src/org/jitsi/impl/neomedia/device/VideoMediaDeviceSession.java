    ...
    implements RTCPFeedbackCreateListener
{
    ...
    private static Component getVisualComponent(Player player)
    {
        ...
        if (player.getState() >= Player.Realized)
        {
            ...
            {
                logger.debug("Called Player#getVisualComponent() "
     ...
     */
    private final List<RTCPFeedbackCreateListener> rtcpFeedbackCreateListeners
        = new LinkedList<>();
    ...
    private Component createLocalVisualComponentForDesktopStreaming()
    {
        ...
        ImageIcon icon
            = (resources == null)
                ? null
                : resources.getImage(DESKTOP_STREAMING_ICON).resolve();
        Canvas canvas;
    ...
    protected boolean fireVideoEvent(
            ...
            boolean wait)
    {
        logger.trace("Firing VideoEvent with type "
    ...
    {
        boolean equal
            = Objects.equals(size, outputSize);
            ...
            Format format)
    {
         ...
         * and not on the operating system. In a perfect world, we will
         * re-implement the functionality below using a Control interface and
