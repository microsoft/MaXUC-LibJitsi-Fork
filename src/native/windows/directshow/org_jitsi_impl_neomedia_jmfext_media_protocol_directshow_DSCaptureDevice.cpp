  ...
  (JNIEnv* env, jobject obj, jlong ptr)
{
    ...
    memcpy((void*)name, (void*)dev->getName(), len * sizeof(jchar));
...
/**
 * \brief Get path of native capture device.
 * \param env JNI environment
 * \param obj DSCaptureDevice object
 * \param ptr native pointer of DSCaptureDevice
 * \return path of the native capture device
 */
JNIEXPORT jstring JNICALL Java_org_jitsi_impl_neomedia_jmfext_media_protocol_directshow_DSCaptureDevice_getPath
  (JNIEnv* env, jobject obj, jlong ptr)
{
    DSCaptureDevice* dev = reinterpret_cast<DSCaptureDevice*>(ptr);
    jstring ret = NULL;
    jsize len = static_cast<jsize>(wcslen(dev->getPath()));
    jchar* path = new jchar[len];

    memcpy((void*)path, (void*)dev->getPath(), len * sizeof(jchar));

    ret = env->NewString(path, len);
    delete[] path;
