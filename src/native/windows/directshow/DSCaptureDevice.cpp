...
DSCaptureDevice::DSCaptureDevice(const WCHAR* name, const WCHAR* path)
{
    if(name)
    {
        m_name = _wcsdup(name);
...
const WCHAR* DSCaptureDevice::getPath() const
{
    return m_path;
}
...
HRESULT DSCaptureDevice::initDevice(IMoniker* moniker)
{
    ...
    if(m_filterGraph)
        return E_FAIL; /* This instance has already been initialized. */
