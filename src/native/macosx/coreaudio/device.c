...
void MacCoreaudio_getChannelsForStereo(
        AudioDeviceID device,
        const char * deviceUID,
        UInt32 * elements,
        UInt32 inputOutputScope);
        ...
        UInt32 inputOutputScope)
{
    ...
    // Get stereo channels for this device
    MacCoreaudio_getChannelsForStereo(device, deviceUID, elements, inputOutputScope);
    ...
    for(i = 0; i < 3; ++i)
    {
        ...
        if(AudioObjectHasProperty(device, &address))
        {
            if((err = AudioObjectSetPropertyData(
                    ...
                    &volume)) != noErr)
            {
                MacCoreaudio_log("MacCoreaudio_setDeviceVolume (coreaudio/device.c):" \
        ...
        {
            MacCoreaudio_log("MacCoreaudio_setDeviceVolume (coreaudio/device.c):" \
                             " AudioObjectHasProperty: Does not support setting volume:" \
                             " for device %s for element: %d - %d",
                             deviceUID, i, elements[i]);
        }
    }
        ...
        UInt32 inputOutputScope)
{
    ...
    if((device = MacCoreaudio_getDevice(deviceUID)) == kAudioObjectUnknown)
    {
        MacCoreaudio_log("MacCoreaudio_getDeviceVolume (coreaudio/device.c):"\
                         " MacCoreaudio_getDevice: %s",
                         deviceUID);
        return -1.0;
    }

    // Get the stereo channels for this device
    MacCoreaudio_getChannelsForStereo(device, deviceUID, elements, inputOutputScope);
    ...
    for(i = 0; i < 3; ++i)
    {
        ...
        {
            MacCoreaudio_log("MacCoreaudio_getDeviceVolume (coreaudio/device.c):" \
                             " AudioObjectHasProperty: Does not support getting volume: " \
                             " for device %s for element %d: %d",
                             deviceUID, i, elements[i]);
        }
    }
...
/**
 * Gets the channels for stereo of a given device.
 *
 * @param device The device to get the channels from.
 * @param deviceUID The UID of the device to get the channels from.
 * @param elements The volume elements to be filled in with the correct values. This
 * must be a 3 item length array (master, left, right).
 * @param inputOutputScope The input/output scope to operate on.
 * (e.g. should we get the input channels or the output channels.)
 */
void MacCoreaudio_getChannelsForStereo(AudioDeviceID device,
                                       const char * deviceUID,
                                       UInt32 *elements,
                                       UInt32 inputOutputScope)
{
    // Master is always the same
    elements[0] = kAudioObjectPropertyElementMaster;
    ...
    if(err == noErr)
    {
        // Supports preferred stereo channels
        elements[1] = channels[0];
        elements[2] = channels[1];
        MacCoreaudio_log("MacCoreaudio_getChannelsForStereo (coreaudio/device.c):" \
                         " device %s supports preferred stereo channels (%d, %d)",
                         deviceUID, channels[0], channels[1]);
    }
    else
    {
        // Doesn't support preferred stereo channels - set them to the default,
        // 1 and 2.
        elements[1] = 1;
        elements[2] = 2;
        MacCoreaudio_log("MacCoreaudio_getChannelsForStereo, err: %d" \
                         " for device %s getting preferred stereo channels",
                         ((int) err),
                         deviceUID);
    }
}
